...
async function main() {
	...
	assert(available.statuses.some(entry => entry.key === "rtk" && entry.text?.includes("RTK 0.28.0")), "expected RTK status");
	...
	assert(fallback.statuses.some(entry => entry.text?.includes("rewrite fallback")), "expected fallback status text");
	...
	assert(unavailable.statuses.some(entry => entry.text === "RTK unavailable"), "expected unavailable status text");
