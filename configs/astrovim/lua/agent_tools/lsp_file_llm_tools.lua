...
local function file_exists(p) return libuv.fs_stat(p) ~= nil end
local function rel_path(abs)
  local cwd = libuv.cwd(); return abs:sub(1, #cwd) == cwd and abs:sub(#cwd + 2) or abs
end
...
local WriteFileTool = {
  ...
  cmds              = {
    ...
    function(_agent, args, _in, cb)
      ...
      if not path or path == "" then return cb({ status = "error", data = "file_path required" }) end
      if content == nil then return cb({ status = "error", data = "content required" }) end
      ...
      local ok, err = pcall(function() vim.fn.writefile(vim.split(content, "\n", { plain = true }), path) end)
      if not ok then return cb({ status = "error", data = "Write failed: " .. tostring(err) }) end

      local bufnr = vim.fn.bufadd(path); vim.fn.bufload(bufnr)
      local ft = vim.filetype.match({ filename = path }); if ft then vim.bo[bufnr].filetype = ft end
...
- Pay attention to LSP diagnostics in the output to ensure code quality.
  ...
  ["output.prompt"] = function(tool)
    ...
    if type(txt) == "table" then txt = vim.islist(txt) and table.concat(txt, "\n") or vim.inspect(txt) end
    if #txt > 200 then txt = txt:sub(1, 200) .. "..." end
    return ("Write to file: %s\n\nContent preview:\n%s\n\nFile will be formatted (async) and analysed by LSP.")
        :format(tool.args and tool.args.file_path or "", txt)
  end,
}
...
- After editing, the file will be formatted via LSP and checked for diagnostics.
  ...
  ["output.prompt"] = function(tool)
    ...
    if #old_txt > 100 then old_txt = old_txt:sub(1, 100) .. "..." end
    if #new_txt > 100 then new_txt = new_txt:sub(1, 100) .. "..." end
    ...
    return ("Edit file: %s%s\n\nReplace:\n%s\n\nWith:\n%s\n\nFile will be formatted and analysed by LSP.")
        :format(tool.args and tool.args.file_path or "", replace_all, old_txt, new_txt)
  end,
}
